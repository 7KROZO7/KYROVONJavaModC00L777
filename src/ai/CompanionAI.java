...
public class CompanionAI extends AIController {
    ...
    public void updateMovement() {
        ...
        if (player != null && player.unit() != null && player.unit().isValid() && player.team() == unit.team) {
            ...
            // If player died and respawned far away (or teleported)
            if (distance > 220f) {
                KVREffects.warpRift.at(unit.x, unit.y); // Exit rift
                unit.set(target.x + 20f, target.y + 20f); // Teleport to new body
                KVREffects.warpRift.at(unit.x, unit.y); // Entry rift
